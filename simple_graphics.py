...
def draw_dirt(x, y, width, height):
    """
    Draws a textured dirt foreground using a mix of multiple shades of brown.
    
    AI Attribution:
    Generated by: Gemini
    Student Prompt: [INSERT YOUR EXACT PROMPT HERE]
    """
    import random
    
    # Base dark brown rectangle for the main dirt area
    _canvas.create_rectangle(x, y, x + width, y + height, 
                             fill="#3b2413", outline="", width=0)
    
    # A list of different shades of brown for the dirt texture
    brown_shades = ["#8b5a2b", "#a0522d", "#cd853f", "#654321", "#4a3018", "#5c4033"]
    
    # Scatter dirt flecks with varying colors
    for _ in range(50):
        fleck_x = x + random.randint(0, width - 10)
        fleck_y = y + random.randint(0, height - 5)
        fleck_w = random.randint(5, 15)
        fleck_h = random.randint(2, 5)
        
        # Pick a random shade of brown for each fleck
        shade = random.choice(brown_shades)
        
        _canvas.create_oval(fleck_x, fleck_y, fleck_x + fleck_w, fleck_y + fleck_h,
                            fill=shade, outline="", width=0)
        
def draw_grass_and_trees(x, y, width, height):
    """
    Draws a layer of solid green grass and randomly places some trees on top.
    ...
    _canvas.create_rectangle(x, y, x + width, y + height, 
                             fill="#32CD32", outline="", width=0) # LimeGreen grass
    
    # Randomly draw between 3 and 6 trees
    num_trees = random.randint(3, 6)
    for _ in range(num_trees):
        # Determine tree size and placement
        tree_x = random.randint(x + 20, x + width - 30)
        trunk_w = random.randint(10, 15)
        trunk_h = random.randint(40, 70)
        leaf_radius = random.randint(25, 40)
        
        # Draw the trunk (sitting right on top of the grass at 'y')
        _canvas.create_rectangle(tree_x, y - trunk_h, tree_x + trunk_w, y, 
                                 fill="#5C4033", outline="", width=0)
        
        # Draw the leaves (centered above the trunk)
        center_x = tree_x + (trunk_w / 2)
        center_y = y - trunk_h
        _canvas.create_oval(center_x - leaf_radius, center_y - leaf_radius, 
                            center_x + leaf_radius, center_y + leaf_radius, 
                            fill="#228B22", outline="", width=0) # ForestGreen leaves
